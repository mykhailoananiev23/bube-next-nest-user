...
export const InboxDropDown = () => {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <Menu.Button className="inline-flex relative items-center p-2 border-l border-[#e5e7eb]">
        <BellIcon className="h-6 w-6" aria-hidden="true" />
        {NewMessage.length !== 0 ? (
          <div
            className={`inline-flex absolute -top-[0.4rem] -right-[0.4rem] justify-center items-center w-5 h-5 text-xs font-bold text-white bg-[#1e50d7] rounded-full`}
          >
            {NewMessage.length}
          </div>
        ) : null}
          ...
          <div className="py-1 max-h-[523px] block overflow-auto scroll-smooth mb-3">
            {NewMessage.length !== 0 &&
              NewMessage.map((ele: any, idx: number) => (
                <InboxUserList data={ele} key={idx} />
              ))}
          </div>
        </Menu.Items>
      </Transition>
